...
const messages: Record<Locale, Messages> = {
  pl: {
    ...
    P12_INVALID: (value, allowed) => `P_12 = "${value}" — nieprawidłowa stawka. Dozwolone: ${allowed}`,
  ...
  en: {
    ...
    GTU_WRONG_FORMAT: (tag) =>
      `<${tag}>1</${tag}> — incorrect GTU format. Use <GTU>${tag}</GTU>`,
    TRAILING_ZEROS: (row, field, value, clean) =>
  ...
  ua: {
    PODMIOT2_JST_MISSING: 'Відсутній елемент JST в Podmiot2 — обов\'язковий в FA(3). Додайте <JST>2</JST>.',
    PODMIOT2_GV_MISSING: 'Відсутній елемент GV в Podmiot2 — обов\'язковий в FA(3). Додайте <GV>2</GV>.',
    P12_INVALID: (value, allowed) => `P_12 = "${value}" — неправильна ставка. Дозволені: ${allowed}`,
    P13_8_REQUIRES_P18: (current) =>
...
export function getMessage(locale: Locale, key: keyof Messages, ...args: any[]): string {
