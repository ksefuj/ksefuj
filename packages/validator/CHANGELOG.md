...
- New semantic rule `CURRENCY_RATE_MISMATCH` (warning) — checks `KursWaluty` against the NBP
  mid-rate to 4 decimal places; fix suggestion includes the exact correct value
...
- Rate fetching is intentionally out of scope for the package — see `apps/web/src/lib/nbp.ts` for
  the reference client implementation including weekend/holiday fallback logic
